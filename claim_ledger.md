...
| C1-02 | 摘要/C1貢獻 | F1=0.9805±0.0004, AP=0.9927±0.0009 | 同上 | 強 | 同上 | 保留 |
| C2-01 | 摘要/C2貢獻/表一之二 | Direct-mixed mIoU=0.9097±0.0553 | e0_direct_mixed_seed{2026,2027,2028}_TARGET_TEST | 強（n=3，target test） | 不解釋為必然負遷移 | 保留 |
...
| C3-01b | 表三之二 | Mean±SD: P=0.8437±0.1346, R=0.9023±0.0743, F1=0.8634±0.0455, IoU_g=0.7615±0.0690, AP=0.9586±0.0026 | e2_ratio_matched_seed{2026,2027,2028} | 中（AP穩定，其餘變異大） | 明確標示複合介入，非單一機制 | 保留 |
...
- E0 val AP（source-only、direct-mixed 三 seed）：val 階段選模從未計算 AP，為 known gap。這些列的 AP 欄位為空白，但均不出現在論文任何 AP 主張中。
- E4 Focal-priority val AP：已於 2026-09-17 補算（AP=0.9903，來源：e0_target_only_seed* val 推論，`compute_missing_ap.py`）。
- E6 percentile seed2026_v2 val AP：已於 2026-09-17 補算（AP=0.9244，來源：`compute_missing_ap.py`）。

## 可追溯性聲明
本表所有 claim 均可在 results_master_formal.csv 找到對應的 experiment_id、seed、split 與 metrics。
